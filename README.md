...
gives 20 tables, all of which are in BCNF: 8 out of the normalization and 12
straight from the mapping.

The EER model carries all five constructs: specialization (EMPLOYEE to
MANAGER/DRIVER, total and disjoint; PRODUCT to PERISHABLE_PRODUCT, partial),
generalization (SHIPMENT + GOODS_RETURN up into GOODS_MOVEMENT), aggregation
(INSPECTION against the boxed SUPPLIER-SUPPLIES-PRODUCT arrangement), the weak
entity ORDER_ITEM, and the multivalued, composite and derived attributes.
Specialization and generalization draw the same and map the same; the
difference is that one reasons downwards and the other upwards.
...
and the aggregation all land in the other twelve tables.
...
11. `final_schema` — all 20 tables with their keys
